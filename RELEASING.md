...
当前下游发行版本为 `0.2.145`。`packaging/upstream.json` 分开记录不可变的
...
2. 为同一审核提交创建并推送 `v0.2.145` 源标签。从该标签手动运行
   `Promote Portable Downstream SDK`，输入 `version=0.2.145` 和
   `source_ref=v0.2.145`；工作流没有可直接选择 PyPI、跳过 TestPyPI 的
   target 参数。如果不可变源标签中的 workflow 本身存在发布前缺陷，
   在 `main` 修复并通过 CI 后创建新的 `v0.2.145-publish.1` runner 标签，
...
3. 批准 `testpypi` Environment。工作流上传同一 artifact 后会自动校验
   ...
   python -m pip download --no-deps \
     ...
     'ink-claude-dream-agent-sdk==0.2.145'
   python -m pip install ./ink_claude_dream_agent_sdk-0.2.145-py3-none-any.whl
